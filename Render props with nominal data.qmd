...
render_props <- function(items, test = "lm", ...) {
  ...
  get_means <- function(x, response, predictor, alpha_level = 0.05) {

    x <- tibble(response = x[[response]], predictor = factor(x[[predictor]]))

    df <- tibble()

    for(level in levels(x$predictor)) {
        model <- lm(response ~ 1, data = x %>% filter(predictor == level))
        df <- bind_rows(
            df,
            tibble(
                level = level,
                mean = model$coefficients %>% as.vector,
    ...
    if(p <= alpha_level) df$stat_sig <- c("*", "") else df$stat_sign <- ""
  ...
  if(test == "chi-square") {
    ...
    df <- items %>%
      pivot_longer(cols = all_of(names(items))) %>%
      group_by(name) %>%
      dplyr::summarize(get_means(name, predictor, alpha_level = options$alpha_level), .groups = "keep") %>%
      ungroup() %>%
      mutate(
        item = factor(
          x = item,
  ...
  if(ncol(items) > 1) {
    ...
    mapping_content <- aes(
      ...
      y = mean, 
      fill = level,
      label = paste0(format(mean, nsmall = 1), "%", stat_sig)
    )
